...
use bitcoincore_rpc::{self as btc_rpc, json as btc_json, RpcApi};
...
fn main() {
    ...
    websocket.listen(
        format!(
            "0.0.0.0:{}",
            std::env::var("PORT").unwrap_or("8080")
        )
    ).unwrap();
}
...
struct WSFactory(
    ...
    Arc<Mutex<Vec<tendermint::validator::Info>>>
);
impl ws::Factory for WSFactory {
    ...
    fn connection_made(&mut self, conn: ws::Sender) -> NullHandler {
        ...
        let update = Update {
            ...
            validators: self.2.lock().unwrap().clone()
        };
...
enum TmTx {
    ...
    WorkProof { value: u64, pubkey: String },
}
...
struct Server {
    ...
    validators: Arc<Mutex<Vec<tendermint::validator::Info>>>
}
...
impl Server {
        ...
        std::thread::spawn(move || {
            ...
            loop {
                ...
                if !btc_blocks.is_empty() || !tm_blocks.is_empty() {
                    let validators = self.get_tm_validators().unwrap();
    ...
    fn get_new_btc_blocks(&mut self) -> Result<Vec<BtcBlock>> {
        let height = self.bitcoin_rpc.get_block_count()?;

        let mut blocks = vec![];
        let start_height = max(self.btc_height(), max(height, 5) - 5) + 1;
    ...
    fn get_new_tm_blocks(&mut self, max_scan: u64) -> Result<Vec<TmBlock>> {
        ...
        let mut blocks = vec![];
        let start_height = max(self.tm_height(), max(height, max_scan) - max_scan) + 1;
    ...
    fn get_tm_validators(&self) -> Result<Vec<tendermint::validator::Info>> {
        let height = self.tm_height();
        let req = tm_rpc::endpoint::validators::Request::new(height.into());
        let validators = self.tendermint_rpc.perform(req)?.validators;
        *self.validators.lock().unwrap() = validators.clone();
        Ok(validators)
    }
}
...
fn connect_btc_rpc(address: &str) -> btc_rpc::Result<btc_rpc::Client> {
    ...
    let user = std::env::var("BTC_RPC_USER").expect("expected env var 'BTC_RPC_USER'");
    let pass = std::env::var("BTC_RPC_PASS").expect("expected env var 'BTC_RPC_PASS'");
